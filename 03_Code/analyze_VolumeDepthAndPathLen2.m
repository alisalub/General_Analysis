...
figure
subplot(1,2,1)
plot(T.(varNames{1}),T.(varNames{3}),'ro')
xlabel(varNames{1})
ylabel(varNames{3})

subplot(1,2,2)
plot(T.(varNames{2}),T.(varNames{3}),'ro')
xlabel(varNames{2})
ylabel(varNames{3})
...
%%  genotype vs depth  
figure
plot(T.(varNames{4}),T.(varNames{2}),'o','MarkerSize',18)
xlabel(varNames{4})
ylabel(varNames{2})

modelStr = sprintf('%s~%s',varNames{[2 4]});
lm = fitlm(T,modelStr);
disp(lm)
...
%%  length vs depth  
figure
plot(T.(varNames{3}),T.(varNames{2}),'o','MarkerSize',18)
xlabel(varNames{3})
ylabel(varNames{2})

modelStr = sprintf('%s~%s',varNames{[2 3]});
lm = fitlm(T,modelStr);
disp(lm)
